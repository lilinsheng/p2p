...
public class CalculatetUtil {
	...
	public static BigDecimal getMonthlyRate(BigDecimal yearRate) {
		...
		return yearRate.divide(ONE_HUNDRED).divide(NUMBER_MONTHS_OF_YEAR, Constants.SCALE_CAL, RoundingMode.HALF_UP);
	}
	...
	public static BigDecimal calTotalInterest(int returnType, BigDecimal bidRequestAmount, BigDecimal yearRate,
		...
		if (returnType == Constants.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			// 只借款一个月
			if (monthes2Return == 1) {
				totalInterest = bidRequestAmount.multiply(monthlyRate).setScale(Constants.SCALE_CAL,
				...
				// 算出每月还款
				BigDecimal monthToReturnMoney = temp1.multiply(temp2).divide(temp3, Constants.SCALE_CAL,
						RoundingMode.HALF_UP);
				// 算出总还款
		...
		} else if (returnType == Constants.RETURN_TYPE_MONTH_INTEREST) {// 按月到期
		...
		return DecimalFormatUtil.formatBigDecimal(totalInterest, Constants.SCALE_STORE);
	}
	...
	public static BigDecimal calMonthlyInterest(int returnType, BigDecimal bidRequestAmount, BigDecimal yearRate,
		...
		if (returnType == Constants.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			// 只借款一个月
			if (monthes2Return == 1) {
				monthlyInterest = bidRequestAmount.multiply(monthlyRate).setScale(Constants.SCALE_CAL,
						RoundingMode.HALF_UP);
			} else {
				BigDecimal temp1 = bidRequestAmount.multiply(monthlyRate);
				BigDecimal temp2 = (BigDecimal.ONE.add(monthlyRate)).pow(monthes2Return);
				...
				if (monthIndex < monthes2Return) {
					monthlyInterest = DecimalFormatUtil
							.amountFormat((temp1.subtract(monthToReturnMoney)).multiply(temp4).add(monthToReturnMoney)
	...
	public static BigDecimal calMonthToReturnMoney(int returnType, BigDecimal bidRequestAmount, BigDecimal yearRate,
		...
		if (returnType == Constants.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			if (monthes2Return == 1) {
				...
				BigDecimal temp3 = (BigDecimal.ONE.add(monthlyRate)).pow(monthes2Return).subtract(BigDecimal.ONE);
				// 算出每月还款
				monthToReturnMoney = temp1.multiply(temp2).divide(temp3, Constants.SCALE_CAL, RoundingMode.HALF_UP);
			}
		} else if (returnType == Constants.RETURN_TYPE_MONTH_INTEREST) {// 按月到期
			BigDecimal monthlyInterest = bidRequestAmount.multiply(monthlyRate).setScale(Constants.SCALE_CAL,
					RoundingMode.HALF_UP);
			if (monthIndex == monthes2Return) {
				monthToReturnMoney = bidRequestAmount.add(monthlyInterest).setScale(Constants.SCALE_CAL,
						RoundingMode.HALF_UP);
			} else if (monthIndex < monthes2Return) {
				monthToReturnMoney = monthlyInterest;
			}
		}
		return DecimalFormatUtil.formatBigDecimal(monthToReturnMoney, Constants.SCALE_STORE);
	}
	
	/**
	 * 计算收款期利息 :当前bid投标金额 / 借款总额 * 当前期还款利息
	 * @param bidAmount :一个bid投标金额
	 * @param bidRequestAmount :总借款金额
	 * @param psMonthlyInterest:对应的还款计划某期利息
	 * @return
	 */
	public static BigDecimal calMonthlyReceiveInterest(BigDecimal bidAmount ,BigDecimal bidRequestAmount,BigDecimal psMonthlyInterest){
		return bidAmount.divide(bidRequestAmount,Constants.SCALE_CAL,BigDecimal.ROUND_HALF_UP).multiply(psMonthlyInterest).setScale(Constants.SCALE_STORE,BigDecimal.ROUND_HALF_UP);
	}
	 ...
	 * @param bidAmount
	 * @param bidRequestAmount
	 * @param psMonthlyPrincipal
	 * @return
	 */
	public static BigDecimal calMonthlyReceivePrincipal(BigDecimal bidAmount ,BigDecimal bidRequestAmount,BigDecimal psMonthlyPrincipal){
		return bidAmount.divide(bidRequestAmount,Constants.SCALE_CAL,BigDecimal.ROUND_HALF_UP).multiply(psMonthlyPrincipal).setScale(Constants.SCALE_STORE,BigDecimal.ROUND_HALF_UP);
	}
	...
	public static BigDecimal calBidInterest(BigDecimal bidRequestAmount, int monthes2Return, BigDecimal yearRate,
		...
		// 所占比例
		BigDecimal proportion = acturalBidAmount.divide(bidRequestAmount, Constants.SCALE_CAL, RoundingMode.HALF_UP);
		BigDecimal bidInterest = totalInterest.multiply(proportion);
		return DecimalFormatUtil.formatBigDecimal(bidInterest, Constants.SCALE_STORE);
	}
	...
	// return
	// DecimalFormatUtil.formatBigDecimal(amount.multiply(Constants.RECHARGE_FEE_RATE),
	// Constants.SCALE_STORE);
	// }
	...
	public static BigDecimal calAccountManagementCharge(BigDecimal bidRequestAmount) {
		BigDecimal accountManagementCharge = DecimalFormatUtil
				.formatBigDecimal(bidRequestAmount.multiply(ACCOUNT_MANAGER_CHARGE_RATE), Constants.SCALE_STORE);
