				...
				$("#editForm").validate({
                    ...
                    submitHandler:function (form) {
                        $(form).ajaxSubmit(function (data) {
                            if (data.success){
                                $.messager.confirm("提示","申请成功",function () {
                                    window.location.href="/borrowInfo";
                                })
                            }else {
					...
					rules : {
						bidRequestAmount:{
							required:true,
							number:true,
							min:${minBidRequestAmount?string("#0.00")},
							max:${account.remainBorrowLimit?string("#0.00")}
						},
						currentRate:{
							required:true,
							number:true,
		...
		<div class="container">
			...
			<form class="form-horizontal el-borrow-form" id="editForm" novalidate="novalidate" method="post" action="/borrow_apply">
				...
				<div class="form-group">
					...
					<div class="col-sm-3 input-group">
						<select class="form-control" name="returnMonthes">
