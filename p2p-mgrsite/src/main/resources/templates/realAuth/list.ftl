...
<script type="text/javascript">
	$(function() {
		...
		//审核模态框
		$(".auditClass").click(function (data) {
			var json = $(this).data("json");
            console.log(json);
            $("#editform #id").val(json.id);
			$("#editform #username").html(json.username);
			$("#editform #realname").html(json.realname);
			$("#editform #idNumber").html(json.idNumber);
			$("#editform #sex").html(json.sex);
			$("#editform #birthDate").html(json.birthDate);
			$("#editform #address").html(json.address);
			$("#editform #image1").attr("src",json.image1);
			$("#editform #image2").attr("src",json.image2);
			$("#myModal").modal("show");
        });
		...
		$(".btn_audit").click(function () {
            ...
            $("#editform").ajaxSubmit(function (data) {
				if (data.success){
                    $.messager.confirm("提示","操作成功，点击确定刷新",function () {
                        window.location.href="/realAuth";
                    })
				}else {
			...
			<div class="col-sm-9">
				...
				<form id="searchForm" class="form-inline" method="post" action="/realAuth">
					...
					<div class="form-group">
						...
						<script type="text/javascript">
							$("option[value=${qo.state}]").prop("selected",true);
						</script>
					</div>
					<div class="form-group">
					    <label>申请时间</label>
					    <input class="form-control" readonly="readonly" type="text" name="beginDate" id="beginDate" value="${(qo.beginDate?string("yyyy-MM-dd"))!}" />到
					    <input class="form-control" readonly="readonly" type="text" name="endDate" id="endDate" value="${(qo.endDate?string("yyyy-MM-dd"))!}" />
					</div>
					<div class="form-group">
		      ...
		      <div class="modal-body">
		      	<form class="form-horizontal" id="editform" method="post" action="/realAuth_audit">
					<fieldset>
						...
						<input type="hidden" name="realAuthId" id="id" value="" />
