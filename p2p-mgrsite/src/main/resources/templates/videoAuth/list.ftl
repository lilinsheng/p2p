...
<script type="text/javascript">
	$(function(){
			...
			form.ajaxSubmit(function(data){
				if(data.success){
					$.messager.confirm("提示","审核成功!",function(){
						window.location.href="/videoAuth";
					});
				}else{
		...
		$(".audit").click(function(){
		    ...
		    $("#editForm #orderTime").html(json.orderTime);
	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-sm-9">
				...
				<div class="row">
					<!-- 提交分页的表单 -->
					<form id="searchForm" class="form-inline" method="post" action="/videoAuth">
						...
						<div class="form-group">
							<label>预约用户</label>
							<input class="form-control" style="width: 130px;" type="text" name="username" id="username"
								   value="${qo.username!''}" />
						</div>
						<div class="form-group">
						...
						<tbody>
							<#list pageResult.listData as vo>
								<tr>
									...
									<td>
										${vo.beginDate?string("yyyy-MM-dd HH:mm")} -
										${vo.endDate?string("HH:mm")}
									</td>
									<td>${vo.stateDisplay}</td>
									<td>${(vo.remark)!''}</td>
									<td>${(vo.auditTime?string("yyyy-MM-dd"))!""}</td>
									<td><#if vo.state==0><a class="audit" data-json='${vo.jsonString}'>审核</a>
									<#elseif vo.state==1>审核通过<#elseif vo.state==2>审核拒绝</#if> </td>
							...
							<div class="form-group">
								...
								<div class="col-sm-6">
									<div class="dropdown" id="autocomplate">
	                                    <label  class="form-control" id="username" autocomplete="off" ></label>
									</div>
								</div>
							</div>
							<div class="form-group">
